...
INTEGER                   :: iNode,jNode,iSide,fNode  ! ?
INTEGER                   :: counter   ! ?
INTEGER                   :: NodeID  ! ?
LOGICAL                   :: dominant  ! ?
LOGICAL                   :: isInner  ! ?
...
DO iSide=1,nInnerSides-1
  ...
  IF(SUM(ABS(SideConnect(1:4,iSide+1)-SideConnect(1:4,iSide))).EQ.0)THEN
    ...
    ELSEIF(Side%tmp2.LT.0) THEN !only connect from periodic slave side to master side (->nSide has a dummy connection side)
      ...
      END DO
      ! Windows/SSE2 fallback: GlobalUniqueNodes leaves %tmp set to the SFC index of the
      ! unique node; merged nodes share the same %tmp value even if ASSOCIATED() returns false.
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(Side%Node(1)%np%tmp .EQ. nSide%connection%Node(iNode)%np%tmp) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
      END IF
      ! Second fallback: unique node ind is positive and equal after merge
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(Side%Node(1)%np%ind .EQ. nSide%connection%Node(iNode)%np%ind) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
      END IF
      ! Last resort: scan all slave side nodes by coordinate
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(SAMEPOINT(Side%Node(1)%np%x,nSide%connection%Node(iNode)%np%x)) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
    ...
    ELSE !Side is master periodic, nSide is slave
      ...
      END DO
      ! Windows/SSE2 fallback: after GlobalUniqueNodes both merged nodes share the same
      ! ConnectMesh NodeID in %tmp; use it when ASSOCIATED() returns false on Windows.
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(nSide%Node(1)%np%tmp .EQ. Side%connection%Node(iNode)%np%tmp) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
      END IF
      ! Fallback: merged unique node ind is equal after GlobalUniqueNodes
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(nSide%Node(1)%np%ind .EQ. Side%connection%Node(iNode)%np%ind) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
      END IF
      ! Last resort: coordinate comparison within mesh tolerance
      IF(fNode.EQ.0) THEN
        DO iNode=1,Side%nNodes
          IF(SAMEPOINT(nSide%Node(1)%np%x,Side%connection%Node(iNode)%np%x)) THEN
            fNode=iNode
            EXIT
          END IF
        END DO
