...
IF(NOT LIBS_BUILD_HDF5)
  ...
  IF(NOT "${HDF5_C_LIBRARY_hdf5_c}" STREQUAL "" AND NOT WIN32)
    IF(APPLE)
...
ELSE()
  ...
  IF(NOT HDF5_FOUND)
    ...
    # Select build command depending on platform
    IF(WIN32)
      SET(HDF5_BUILD_CMD ${CMAKE_COMMAND} --build <BINARY_DIR> -j${N})
    ELSE()
      SET(HDF5_BUILD_CMD make -j${N})
    ENDIF()
    ...
    EXTERNALPROJECT_ADD(HDF5
      ...
      # WARNING: The order of the following libraries matters! They need to be listed from the most dependent to the least dependent.
      BUILD_BYPRODUCTS ${LIBS_HDF5_DIR}/lib/libhdf5_fortran.a ${LIBS_HDF5_DIR}/lib/libhdf5_f90cstub.a ${LIBS_HDF5_DIR}/lib/libhdf5_hl_fortran.a ${LIBS_HDF5_DIR}/lib/libhdf5_hl_f90cstub.a ${LIBS_HDF5_DIR}/lib/libhdf5_hl.a ${LIBS_HDF5_DIR}/lib/libhdf5.a ${LIBS_HDF5_DIR}/lib/libhdf5_tools.a
    )
...
ELSE()
  ...
  IF (LIBS_BUILD_MATH_LIB_VENDOR STREQUAL "LAPACK")
    ...
    IF (NOT EXISTS "${LIBS_MATH_DIR}/lib/liblapack.a")
      # Select build command depending on platform
      IF(WIN32)
        SET(LAPACK_BUILD_CMD ${CMAKE_COMMAND} --build <BINARY_DIR> -j${N})
      ELSE()
        SET(LAPACK_BUILD_CMD make -j${N})
      ENDIF()
      ...
      EXTERNALPROJECT_ADD(${LIBS_BUILD_MATH_LIB_VENDOR}
        ...
        UPDATE_COMMAND     ""
        BUILD_COMMAND      ${LAPACK_BUILD_CMD}
  ...
  ELSE()
    ...
    IF(NOT CGNS_FOUND)
      ...
      EXTERNALPROJECT_ADD(cgns
        ...
        # Pass HDF5_DIR so the sub-build finds the same HDF5 as the parent (needed when LIBS_BUILD_HDF5=OFF uses system HDF5)
        CMAKE_ARGS         -DCMAKE_INSTALL_PREFIX=${LIBS_CGNS_DIR} -DCMAKE_PREFIX_PATH=${LIBS_HDF5_DIR} -DHDF5_DIR=${HDF5_DIR} -DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} -DCMAKE_Fortran_COMPILER=${CMAKE_Fortran_COMPILER} -DCGNS_ENABLE_FORTRAN=ON -DCGNS_ENABLE_64BIT=${LIBS_CGNS_64BIT} -DCGNS_BUILD_SHARED=OFF -DCGNS_USE_SHARED=OFF -DCMAKE_BUILD_TYPE=Release -DCGNS_BUILD_CGNSTOOLS=OFF -DCGNS_ENABLE_HDF5=ON -DHDF5_NEED_MPI=${HDF5_IS_PARALLEL} -DCGNS_ENABLE_PARALLEL=OFF -DCGNS_ENABLE_TESTS=OFF -DCMAKE_SKIP_RPATH=ON
        # Set the build byproducts
...
# OpenMP
# =========================================================================
OPTION(LIBS_USE_OPENMP "Enable OpenMP parallelization" ON)
IF(LIBS_USE_OPENMP)
  FIND_PACKAGE(OpenMP)
  IF(OpenMP_Fortran_FOUND)
    MESSAGE(STATUS "Compiling with [OpenMP] (v${OpenMP_VERSION})")
    SET(CMAKE_Fortran_FLAGS "${CMAKE_Fortran_FLAGS} ${OpenMP_Fortran_FLAGS}")
    LIST(APPEND linkedlibs ${OpenMP_Fortran_LIBRARIES})
  ELSE()
